...
var ecsideObj=ECSideUtil.getGridObj(formid);
	...
	var form=ecsideObj.ECForm;
	if(selectObj.className=="combobox-f combo-f"){
		form[formid+"_crd"].value=$(selectObj).combobox('getValue');
	}else if("object" == typeof(selectObj)){
		form[formid+"_crd"].value=selectObj.options[selectObj.selectedIndex].value;
	}else{
		form[formid+"_crd"].value=selectObj;	
	}
	form[pageid].value='1';
...
/*
   ...
   功能：ecside截取（恢复）字符串
   参数：obj,当前div对象
       divWidth,定义超过多少像素为省略号
*/
function ec_showContent(obj,divWidth){
	if(obj.style.width==divWidth && obj.className=="ellipsis")
    {
    	obj.style.width="100%";
    	obj.className="";

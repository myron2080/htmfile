	...
	var _appendSubMenu = function($parent, $iframe, basePath, mainMenu, pid) {
		...
		for (var i = 0; i < menuItems.length; i++) {
			...
			if (menuItem.url == null || menuItem.url == "") {
				...
				// 有url情况下跳转至url
				// TODO id全部带_，不使用.。
				var menuId = menuItem.menuId.replace(/_/g, ".");
...
//		var menuInfoId = params.menuInfoId == null ? defaults.menuInfoId : params.menuInfoId;
		...
		// 获取信息
		// TODO 待删除
		var str = $menuInfo.val().replace(/\./g, "_");
		...
		for (var i = 0; i < modeNavArr.length; i++) {
			...
			$('#' + modeBtn.menuId).on('click', {
				...
				$('.menu-mode-nav-li').attr('style','background:#C4C4C4');
				$(this).attr('style','background:#778899');
	...
	*/
	var initSearch = function(jq, params) {
		...
		for (var i = 0; i < searchItems.length; i++) {
			...
			var html = "<span class='search-item-div'>" +
				"<span class='search-item-label' style='width:" + titleWidth + "px;'>" + item.title + "</span>" +
				"<input id='" + itemId + "' name='" + item.field + "' class='search-item-label' style='width:" + inputWidth + "px;'/>" +
				"</span>";
			$search.append(html);
		}
